...
(function (root, factory) {
    ...
    Calendar.prototype = {
        ...
        init: function (settings) {
            $('#calendar2').append('<div class="ui-calendar clearfix" id="' + this.id + '"><div class="ui-calendar-pannel clearfix" data-role="pannel"><span class="ui-calendar-control" data-role="prev-year">&lt;&lt;</span><span class="ui-calendar-control" data-role="prev-month">&lt;</span><span class="ui-calendar-control month" data-role="current-month"></span><span class="ui-calendar-control year" data-role="current-year"></span><span class="ui-calendar-control" data-role="next-month">&gt;</span><span class="ui-calendar-control" data-role="next-year">&gt;&gt;</span></div><div class="calendar-header clearfix"></div><div class="c_days clearfix"></div></div>');
            ...
            // this.formatDate();
            // this.renderHeader();
            if (this.settings.sdate && this.settings.edate) {
                this.initFocusRange(this.settings.sdate, this.settings.edate);
            }
        ...
        formatDate: function () {
            ...
            var predayCount = firstDay;
            if (firstDay > 0) {
                list += this._getDay(preDateNum - firstDay + 1, preDateNum, "preday", preDate);
            }

            if (predayCount + dayNum >= 35) {
                //没有nextday,需要截取
                var dayCount = 35 - predayCount;
                list += this._getDay(1, dayCount, '', date);
            }
            else {
                //有nextday
        ...
        _getDay: function (startNum, dayNum, cls, date) {
            ...
            for (var i = startNum; i <= dayNum; i++) {
                ...
                } else if (time == +new Date(this.defaultDate.getFullYear(), this.defaultDate.getMonth(), this.defaultDate.getDate())) {
                    className += ' focus';
                }
        ...
        setfocusRange: function (value) {
            ...
            //找到dateArr中的最大值和最小值
            var minDate = Math.min.apply(null, _this.dateArr);
            var maxDate = Math.max.apply(null, _this.dateArr);
            var clickDate;

            _this.setDate(value);
            clickDate = _this.defaultDate.getTime();
            minDate = Math.min.apply(null, _this.dateArr);
            maxDate = Math.max.apply(null, _this.dateArr);

            //单击的位置在两边

            if (maxDate != minDate) {
                if (clickDate == minDate || clickDate == maxDate) {

                    for (i = minDate + 1000 * 60 * 60 * 24; i < maxDate; i = i + 1000 * 60 * 60 * 24) {
                        flagExist = false;

                        for (j = 0; j < _this.dateArr.length; j++) {
                            if (i == _this.dateArr[j].getTime()) {
                                flagExist = true;
                            }
                        }

                        if (!flagExist) {
                            _this.dateArr.push(new Date(i));
                        }
                ...
                if (clickDate > minDate && clickDate < maxDate) {
                    //删除数组中比它小的时间
                    for (j = 0; j < _this.dateArr.length; j++) {
                        if (clickDate > _this.dateArr[j].getTime()) {
                            _this.dateArr.splice(j, 1);
                            j--;
                        }
                    }
        ...
        initFocusRange: function (sdate, edate) {
            var sdatem, edatem;
            var _this = this;
            var i;
            if (sdate != edate) {
                _this.setDate(sdate)
                sdatem = _this.defaultDate.getTime();
                _this.setDate(edate);
                edatem = _this.defaultDate.getTime();
                for (i = sdatem; i <= edatem; i = i + 1000 * 60 * 60 * 24) {
                    _this.dateArr.push(new Date(i));
                }
            }
        }
